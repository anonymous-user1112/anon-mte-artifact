...
void write_random_order(uint64_t* ptr, uint64_t count, LFSR &lfsr, int workload_iter) {
    for(int j = 0; j<workload_iter; j++){
        for (int i = 0; i < count; i++){
            bool result = xor25(lfsr);
            lfsr.rightShift(result);
            uint32_t index = lfsr.getArray();  
            index = prng(i) * index; 
            ptr[index%count] = (uint64_t)i+j;
        }
    }
}

// // write after write: but dmb
...
void read_random_order(uint64_t* ptr, uint64_t count, LFSR &lfsr, int workload_iter) {
    for(int j = 0; j<workload_iter; j++){
        for (int i = 0; i < count; i++){
            ...
            uint32_t index = lfsr.getArray();  
            index = prng(i) * index; 
            FORCE_READ_INT(ptr[index % count]);
        }
    }
}
...
void store_load_random_order(uint64_t* ptr, uint64_t count, LFSR &lfsr, int workload_iter) {
    for(int j = 0; j<workload_iter; j++){
        ...
        for (int i = 0; i < count; i++){
            ...
            lfsr.rightShift(result);
            uint32_t index = lfsr.getArray();  
            index = (prng(i) * index) % count; 
            ptr[index] = ptr[old_index] + i + j;
            old_index = index;
        }
    }
}
