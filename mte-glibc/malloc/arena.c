		 ...
		 int mmap_flags)
{
  ...
  if (aligned_heap_area)
    {
      p2 = (char *) MTE_MMAP (aligned_heap_area, max_size, PROT_NONE, mmap_flags);
      aligned_heap_area = NULL;
      if (p2 != MAP_FAILED && ((unsigned long) p2 & (max_size - 1)))
        {
          MTE_MUNMAP (p2, max_size);
          p2 = MAP_FAILED;
        }
    }
  if (p2 == MAP_FAILED)
    {
      p1 = (char *) MTE_MMAP (0, max_size << 1, PROT_NONE, mmap_flags);
      if (p1 != MAP_FAILED)
        {
          ...
          MTE_MUNMAP (p2 + max_size, max_size - ul);
        }
      else
        {
          /* Try to take the chance that an allocation of only max_size
             is already aligned. */
          p2 = (char *) MTE_MMAP (0, max_size, PROT_NONE, mmap_flags);
          if (p2 == MAP_FAILED)
            return 0;
...
shrink_heap (heap_info *h, long diff)
{
  ...
  if (__glibc_unlikely (check_may_shrink_heap ()))
    {
      if ((char *) MTE_MMAP ((char *) h + new_size, diff, PROT_NONE,
