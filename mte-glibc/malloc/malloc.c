...
#define get_tagptr(ptr) ((uintptr_t)ptr >> 56)
#define set_tagptr(ptr, tag) ((uintptr_t)(ptr) | (uintptr_t)tag << 56)
#define remove_tagptr(ptr) ((uintptr_t)ptr & (((uintptr_t)0x1 << 56) - 1))

static void pool__mmap_init (void);
static void * pool__mmap (void *addr, size_t len, int prot, int flags, int fd, __off64_t offset);
static int pool__munmap (void *addr, size_t size);

static bool pool_no_pretag = false;
static uintptr_t pool_tag = 1;
static bool pool_no_reuse = false;
...
static void
init_mte (void) {
  __always_fail_morecore = true; // force it to not use sbrk()

  if (getenv("NO_PRETAG")) {
    pool_no_pretag = true;
    pool_tag = 0;
    // prctl should be called by glibc's routine.
    if (getenv("LAZYTAG")) {
      mtag_enabled = true;
  ...
  pool__mmap_init(); // we use pool no matter what, but pooling should be after prctl
...
static void
fini_mte (void) {
  fprintf(stderr, "sizeof(clock_t) = %lu\n", sizeof(clock_t));
  fprintf(stderr, "tick frequency is: %lu\n", freq=sysconf(_SC_CLK_TCK));

  if ((end = times(&t)) < 0) {	/* mark the end time */
    perror("times");	/* error - print a message and exit */
    exit(1);
  }
  fprintf(stderr, "elapsed time: %ju - %ju = %ju ticks\n", end, start, end-start);
  fprintf(stderr, "elapsed time: %5.2f seconds\n", (double)(end-start)/freq);
}

#define NPAGE_POOL 1024L * 1024L
// #define NPAGE_HUGEPAGE_POOL 1024
#define PGSIZE 4096
// #define HPGSIZE 1024 * 1024
static char *pool_mm;
// static char *pool_hpg_mm;
static bool used[NPAGE_POOL] = {0,};
// static bool used_hugepage[NPAGE_POOL] = {0,};
static bool tagged_page_pool_init = false;

static void
pool__mmap_init (void) {
  if (tagged_page_pool_init)
    return;
  tagged_page_pool_init = true;

  pool_mm = (char *) MMAP (0, PGSIZE * NPAGE_POOL,
      (pool_no_pretag ? mtag_mmap_flags : PROT_MTE) | PROT_READ | PROT_WRITE, 0);
  if (pool_mm == MAP_FAILED)
    exit(1);
  debug_printf ("pool allocation success - %u * %u, POOL_START: %p\n",
      PGSIZE, NPAGE_POOL, pool_mm);

  if (!pool_no_pretag)
    __libc_mtag_tag_region((void *)set_tagptr(pool_mm, pool_tag), PGSIZE * NPAGE_POOL);
  memset(used, 0, sizeof(bool) * NPAGE_POOL);
}

static void *
pool__mmap (void *__addr, size_t len, int __prot, int __flags, int __fd, __off64_t __offset) {
  if (!tagged_page_pool_init)
    pool__mmap_init();

  debug_printf ("[%s] %p %d %d %d %ld\n", __func__, __addr, __prot, __flags, __fd, __offset);
  // size_t pagesize = len > HPGSIZE ? HPGSIZE : PGSIZE;
  // void *_pool = len > HPGSIZE ? pool_start : pool_hugepage_start;
  // bool *_used = len > HPGSIZE ? used : used_hugepage;
  // long int pool_size = len > HPGSIZE ? NPAGE_POOL : NPAGE_HUGEPAGE_POOL;
  size_t pagesize = PGSIZE;
  void *_pool = pool_mm;
  bool *_used = used;
  long int pool_size = NPAGE_POOL;
  if (_pool == NULL || _used == NULL)
    return MAP_FAILED;

  long int requested_pages = ALIGN_UP (len, pagesize) / pagesize;
  debug_printf ("[%s] len: %u, pagesize: %u, npages: %u\t", __func__, len, pagesize, requested_pages);
  int i = 0;
  while (i < pool_size) {
    i++;
    if (_used[i]) continue;
    bool found = true;
    int next = i;
    for (size_t j = 0; j < requested_pages && i + j < pool_size; j++) {
      if (_used[i + j]) {
        found = false;
        next = i + j;
        break;
    ...
    if (found) {
      ...
      memset((void *)set_tagptr(_pool + i * pagesize, pool_tag), 0, len);
      debug_printf ("assigning %lu bytes(%lu pages) from %p[%lu:%lu]\n", len, requested_pages, _pool + i * pagesize, i, i + requested_pages);
      return (void *)set_tagptr(_pool + i * pagesize, pool_tag);
    } else {
      i = next;
    }
  }
...
static int
pool__munmap (void *addr, size_t size) {
  if (!tagged_page_pool_init)
    exit(1);
  ...
  if (get_tagptr(addr) != pool_tag) {
    debug_printf ("pool_munmap received wrong pointer\n", get_tagptr(addr));
    exit(1);
  }
  void *untagged_addr = (void *)remove_tagptr(addr);
  size_t start_idx = (size_t)((uintptr_t)untagged_addr - (uintptr_t)_pool) / pagesize;
  size_t end_idx = (size_t)((uintptr_t)untagged_addr + (uintptr_t) size - (uintptr_t)_pool) / pagesize;
  if (!(start_idx > 0 && end_idx > start_idx && end_idx <= pool_size)) {
    debug_printf ("pool_munmap: requested ptr(%p, idx = [%u:%u]) is not from the pool\n", untagged_addr, start_idx, end_idx);
    exit(1);
  }
  if (pool_no_reuse) return 0;
  for (size_t i = start_idx; i < end_idx; i++) {
    _used[i] = false; // release
  }
  debug_printf ("pool_munmap(%p, %u) freed pool_mm[%u:%u]\n", untagged_addr, size, start_idx, end_idx);
  return 0;
}

#define POOL_MMAP(addr, size, prot, flags) \
  pool__mmap((addr), (size), (prot), (flags)|MAP_ANONYMOUS|MAP_PRIVATE, -1, 0)
#define POOL_MUNMAP(addr, size) \
  pool__munmap(addr, size)

static void *
pool__mremap (void *addr, size_t old_len,
    size_t new_len, int __flags) {
  POOL_MUNMAP (addr, old_len);
  void *newaddr = POOL_MMAP (0, new_len, 0, 0);
  debug_printf ("pool_mremap(%p, %u) get new addr (%p, %u)\n", addr, old_len, newaddr, new_len);
  if (addr != newaddr)
    memcpy(newaddr, addr, old_len);
  return newaddr;
}
#define POOL_MREMAP(addr, oldsize, newsize, flags) \
  pool__mremap(addr, oldsize, newsize, flags)
#endif

#ifdef MTE_PRETAG
#define MTE_MMAP POOL_MMAP
#define MTE_MUNMAP POOL_MUNMAP
#define MTE_MREMAP POOL_MREMAP
#else
#define MTE_MMAP MMAP
#define MTE_MUNMAP __munmap
#define MTE_MREMAP __mremap
#endif
...
/*
   malloc_chunk details:
    ...
    Wilson ftp://ftp.cs.XXXXXXXX.edu/pub/garbage/allocsrv.ps for a
			 ...
			 size_t pagesize, int extra_flags, mstate av)
{
  ...
  char *mbrk = (char *) (MTE_MMAP (0, size,
			       mtag_mmap_flags | PROT_READ | PROT_WRITE,
			       extra_flags));
  if (mbrk == MAP_FAILED)
    return MAP_FAILED;
...
munmap_chunk (mchunkptr p)
{
  ...
  MTE_MUNMAP ((char *) block, total_size);
}
...
mremap_chunk (mchunkptr p, size_t new_size)
{
  ...
  cp = (char *) MTE_MREMAP ((char *) block, total_size, new_size,
                          MREMAP_MAYMOVE);
