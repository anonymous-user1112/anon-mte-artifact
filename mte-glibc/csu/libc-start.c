...
#define SHADOW_OFFSET (1ULL << 38)      // 0x40_0000_0000, android specific
#define SHADOW_SCALE  4                 // 1 shadow byte per 16 bytes

void reserve_shadow(size_t app_mem_size) {
    size_t shadow_size = app_mem_size >> SHADOW_SCALE;
    void *p = mmap((void*)SHADOW_OFFSET, shadow_size,
...
#ifdef MTE_ANALOGUES_HAKC
  // shadow memory can get accessed after this point.
  init_shadow();
#endif /* MTE_ANALOGUES_HAKC */
