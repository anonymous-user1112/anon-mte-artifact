...
#include <algorithm>
#include <atomic>
#if defined(RLBOX_MEASURE_TRANSITION_TIMES) || defined(RLBOX_TIMER_SUPPORT)
#  include <chrono>
#endif
#include <cstdlib>
#include <limits>
...
#if defined(RLBOX_TIMER_SUPPORT)

  auto start_timer() {
    return high_resolution_clock::now();
  }

  void end_timer(std::chrono::time_point<std::chrono::high_resolution_clock>& start_time) {
    auto exit_time = high_resolution_clock::now();
    int64_t ns = duration_cast<nanoseconds>(exit_time - start_time).count();
    transition_times.push_back(rlbox_transition_timing{
        rlbox_transition::INVOKE, "Timer", nullptr, ns });
  }
#endif
