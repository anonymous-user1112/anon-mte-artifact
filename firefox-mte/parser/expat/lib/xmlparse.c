...
typedef struct block {
  ...
  int size;
  XML_Char s[1]
  // MTE CHANGE: Align s to 16 byte offset in the overall allocation.  Since
  // allocations are 16-byte aligned by glibc on the heap, this would mean that
  // this struct is already 16 byte aligned. With this change s is also 16-byte aligned.
...
// MTE CHANGE: Modify poolFinish to align pool->ptr before assignment
#define poolFinish(pool) ((pool)->start = (next_aligned_pool_ptr(pool), (pool)->ptr))
////
...
// MTE CHANGE: Helper functions
static int is_16_aligned_val(uintptr_t val) {
  return (val % 16) == 0;
}
static uintptr_t next_16_aligned_val(uintptr_t val) {
  val += 16 - (val % 16);
  if (!is_16_aligned_val(val)) {
    printf("!!!!!!!!! Expat Alignment failed1\n");
    abort();
  }
  return val;
}
static void next_aligned_pool_ptr(STRING_POOL* pool) {
  if (!is_16_aligned_val((uintptr_t)pool->ptr)) {
    // Its always safe to move the pointer to next aligned 16 value since every
    // block size is a multiple of 16.
    XML_Char* new_ptr = (XML_Char*) next_16_aligned_val((uintptr_t)pool->ptr);
    // Do a sanity check just in case. This value should at most point to the location just after the buffer
    XML_Char* after_buffer_ptr = pool->blocks->s + pool->blocks->size;
    if (new_ptr > after_buffer_ptr) {
      printf("!!!!!!!!! Expat Alignment failed2\n");
      abort();
    }
    pool->ptr = new_ptr;
  }
...
poolBytesToAllocateFor(int blockSize)
{
  ...
  */
...

  {
    ...
    int bytesToAllocate = (int)(
        offsetof(BLOCK, s) + (unsigned)stretchedBlockSize);
    if (bytesToAllocate < 0)
      return 0;

    // MTE CHANGE: Align block sizes to 16
    if (!is_16_aligned_val(bytesToAllocate)) {
      bytesToAllocate = next_16_aligned_val(bytesToAllocate);
    }
    ////
