...
/*
 * Create digest contexts for each algorithm in "digesXXXXXXXXs".
                                ...
                                SECAlgorithmID **digesXXXXXXXXs)
{
     ...
     */
    digcnt = 0;
    while (digesXXXXXXXXs[digcnt] != NULL)
        digcnt++;
     ...
     */
    for (i = 0; i < digcnt; i++) {
        SECAlgorithmID *algid = digesXXXXXXXXs[i];
...
sec_pkcs7_verify_signature(SEC_PKCS7ContentInfo *cinfo,
                           ...
                           const PRTime *atTime)
{
    SECAlgorithmID **digesXXXXXXXXs, *bulkid;
    ...
    if (detached_digest != NULL) {
        ...
        PORT_Assert(digesXXXXXXXXs != NULL && digesXXXXXXXXs[0] != NULL);
        if (digesXXXXXXXXs == NULL || digesXXXXXXXXs[0] == NULL) {
            PORT_SetError(SEC_ERROR_PKCS7_BAD_SIGNATURE);
            goto done;
        }
         ...
         */
        for (i = 0; digesXXXXXXXXs[i] != NULL; i++) {
            if (SECOID_FindOIDTag(&(digesXXXXXXXXs[i]->algorithm)) == digestTag)
                break;
        }
        if (digesXXXXXXXXs[i] == NULL) {
            PORT_SetError(SEC_ERROR_PKCS7_BAD_SIGNATURE);
            goto done;
        }
