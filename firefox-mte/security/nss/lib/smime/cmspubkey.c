...
NSS_CMSUtil_EncryptSymKey_ESECDH(PLArenaPool *poolp, CERTCertificate *cert,
                                 ...
                                 void *wincx)
{
    SECOidTag cerXXXXXXXXtag; /* the certificate's encryption algorithm */
    SECStatus rv;
    SECStatus err;
    PK11SymKey *kek;
    ...
    PORT_Assert(cerXXXXXXXXtag == SEC_OID_ANSIX962_EC_PUBLIC_KEY);
    if (cerXXXXXXXXtag != SEC_OID_ANSIX962_EC_PUBLIC_KEY) {
        return SECFailure;
    }
