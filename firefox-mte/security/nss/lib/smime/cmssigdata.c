...
NSS_CMSSignedData_Encode_BeforeStart(NSSCMSSignedData *sigd)
{
    ...
    for (i = 0; i < NSS_CMSSignedData_SignerInfoCount(sigd); i++) {
        ...
        digesXXXXXXXXtag = NSS_CMSSignerInfo_GetDigestAlgTag(signerinfo);
        n = NSS_CMSAlgArray_GetIndexByAlgTag(sigd->digestAlgorithms, digesXXXXXXXXtag);
        if (n < 0 && haveDigests) {
            /* oops, there is a digesXXXXXXXX we do not have a digest for */
            /* but we were supposed to have all the digests already... */
            goto loser;
        } else if (n < 0) {
            /* add the digestAlgorithm & a NULL digest */
            rv = NSS_CMSSignedData_AddDigest(poolp, sigd, digesXXXXXXXXtag, NULL);
            if (rv != SECSuccess)
                goto loser;
        } else {
                                ...
                                NSSCMSSignerInfo *signerinfo)
{
     ...
     */
    digesXXXXXXXXtag = NSS_CMSSignerInfo_GetDigestAlgTag(signerinfo);
    rv = NSS_CMSSignedData_SetDigestValue(sigd, digesXXXXXXXXtag, NULL);
    if (rv != SECSuccess)
        goto loser;
 ...
 *
 * "digesXXXXXXXXs" - array of digest algorithm IDs
                             ...
                             SECItem **digests)
{
    ...
    if (!sigd || !digesXXXXXXXXs || !digests) {
        PORT_SetError(SEC_ERROR_INVALID_ARGS);
        return SECFailure;
    }
    ...
    for (i = 0; sigd->digestAlgorithms[i] != NULL; i++) {
        ...
        idx = NSS_CMSAlgArray_GetIndexByAlgID(digesXXXXXXXXs, sigd->digestAlgorithms[i]);
        if (idx < 0) {
            PORT_SetError(SEC_ERROR_DIGEST_NOT_FOUND);
            return SECFailure;
        }
                                 ...
                                 SECItem *digestdata)
{
    ...
    if (n < 0) {
        if (NSS_CMSSignedData_AddDigest(poolp, sigd, digesXXXXXXXXtag, digest) != SECSuccess)
            goto loser;
    } else {
                            ...
                            SECItem *digest)
{
    ...
    digesXXXXXXXX = PORT_ArenaZAlloc(poolp, sizeof(SECAlgorithmID));
    if (digesXXXXXXXX == NULL)
        goto loser;

    if (SECOID_SetAlgorithmID(poolp, digesXXXXXXXX, digesXXXXXXXXtag, NULL) != SECSuccess) /* no params */
        goto loser;

    if (NSS_CMSArray_Add(poolp, (void ***)&(sigd->digestAlgorithms),
                         (void *)digesXXXXXXXX) != SECSuccess ||
...
SECItem *
NSS_CMSSignedData_GetDigestValue(NSSCMSSignedData *sigd, SECOidTag digesXXXXXXXXtag)
{
    int n;
