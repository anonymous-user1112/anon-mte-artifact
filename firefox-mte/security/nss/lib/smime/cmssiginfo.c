...
nss_cmssignerinfo_create(NSSCMSMessage *cmsg, NSSCMSSignerIDSelector type,
                         ...
                         SECKEYPrivateKey *signingKey, SECOidTag digesXXXXXXXXtag);
                                      ...
                                      SECKEYPrivateKey *signingKey, SECOidTag digesXXXXXXXXtag)
{
    return nss_cmssignerinfo_create(cmsg, NSSCMSSignerID_SubjectKeyID, NULL,
                                    subjKeyID, pubKey, signingKey, digesXXXXXXXXtag);
}

NSSCMSSignerInfo *
NSS_CMSSignerInfo_Create(NSSCMSMessage *cmsg, CERTCertificate *cert, SECOidTag digesXXXXXXXXtag)
{
    return nss_cmssignerinfo_create(cmsg, NSSCMSSignerID_IssuerSN, cert, NULL,
                                    NULL, NULL, digesXXXXXXXXtag);
}
                         ...
                         SECKEYPrivateKey *signingKey, SECOidTag digesXXXXXXXXtag)
{
    ...
    if (SECOID_SetAlgorithmID(poolp, &signerinfo->digestAlg, digesXXXXXXXXtag, NULL) != SECSuccess)
        goto loser;
                       ...
                       SECItem *contentType)
{
    ...
    if (signerinfo->authAttr != NULL) {
        ...
        rv = SGN_Digest(privkey, digesXXXXXXXXtag, &signature, digest);
    }
                         ...
                         SECItem *contentType) /* may be NULL */
{
    ...
    */
    ...
    if ((pubkAlgTag == SEC_OID_UNKNOWN) || (digesXXXXXXXXtag == SEC_OID_UNKNOWN) ||
    ...
    if (!NSS_CMSArray_IsEmpty((void **)signerinfo->authAttr)) {
        ...
        if (sigAlgTag == pubkAlgTag) {
            ...
            vs = (VFY_VerifyDataDirect(encoded_attrs.data, encoded_attrs.len,
                                       ...
                                       digesXXXXXXXXtag, NULL, signerinfo->cmsg->pwfn_arg) != SECSuccess)
            ...
            if (VFY_VerifyDataWithAlgorithmID(encoded_attrs.data,
                                              ...
                                              &(signerinfo->digestEncAlg), &digesXXXXXXXXtagCmp,
                                              signerinfo->cmsg->pwfn_arg) != SECSuccess) {
                vs = NSSCMSVS_BadSignature;
            } else if (digesXXXXXXXXtagCmp != digesXXXXXXXXtag) {
        ...
        if (sigAlgTag == pubkAlgTag) {
            ...
            vs = (!digest ||
                  VFY_VerifyDigestDirect(digest, publickey, sig, pubkAlgTag,
                                         digesXXXXXXXXtag, signerinfo->cmsg->pwfn_arg) != SECSuccess)
                     ? NSSCMSVS_BadSignature
                     : NSSCMSVS_GoodSignature;
        } else {
            vs = (!digest ||
                  VFY_VerifyDigestWithAlgorithmID(digest, publickey, sig,
                                                  &(signerinfo->digestEncAlg), digesXXXXXXXXtag,
                                                  signerinfo->cmsg->pwfn_arg) != SECSuccess)
                     ? NSSCMSVS_BadSignature
                     : NSSCMSVS_GoodSignature;
        }
...
NSS_CMSSignerInfo_AddCounterSignature(NSSCMSSignerInfo *signerinfo,
                                      SECOidTag digesXXXXXXXX, CERTCertificate signingcert)
{
    /* XXXX TBD XXXX */
    return SECFailure;
}
