                            ...
                            SECItem *DERinput)
{
    ...
    cerXXXXXXXXtag = SECOID_GetAlgorithmTag(&(spki->algorithm));
...
NSS_CMSRecipient_IsSupported(CERTCertificate *cert)
{
    ...
    SECOidTag cerXXXXXXXXtag = SECOID_GetAlgorithmTag(&(spki->algorithm));
                                 ...
                                 SECOidTag bulkalgtag)
{
    ...
    switch (cerXXXXXXXXtag) {
        case SEC_OID_PKCS1_RSA_ENCRYPTION:
            ...
            rv = SECOID_SetAlgorithmID(poolp, &(ri->ri.keyTransRecipientInfo.keyEncAlg), cerXXXXXXXXtag, NULL);
            break;
        case SEC_OID_ANSIX962_EC_PUBLIC_KEY:
            ...
            if (SECOID_SetAlgorithmID(poolp, &oiok->id.originatorPublicKey.algorithmIdentifier,
                                      cerXXXXXXXXtag, NULL) != SECSuccess) {
                rv = SECFailure;
                break;
            }
