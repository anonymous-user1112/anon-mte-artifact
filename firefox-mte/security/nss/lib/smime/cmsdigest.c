...
NSS_CMSDigestContext_StartMultiple(SECAlgorithmID **digesXXXXXXXXs)
{
    ...
    digcnt = (digesXXXXXXXXs == NULL) ? 0 : NSS_CMSArray_Count((void **)digesXXXXXXXXs);
    ...
    */
    ...
    for (i = 0; i < digcnt; i++) {
        ...
        if (!NSS_SMIMEUtil_SigningAllowed(digesXXXXXXXXs[i])) {
            goto loser;
        }
        digobj = NSS_CMSUtil_GetHashObjByAlgID(digesXXXXXXXXs[i]);
        /*
...
{
    SECAlgorithmID *digesXXXXXXXXs[] = { NULL, NULL }; /* fake array */

    digesXXXXXXXXs[0] = digesXXXXXXXX;
    return NSS_CMSDigestContext_StartMultiple(digesXXXXXXXXs);
}
