...
add_task(async function () {
  ...
  async function testHeuristicResult(expectedText, setDefaultSuggestion) {
    ...
    Assert.equal(
      ...
      `XXXXXXXXX ${text}`,
      `Expected heuristic result to have displayurl: "XXXXXXXXX ${text}".`
    );
  ...
  async function testSuggestions(info) {
    ...
    for (let { content, description } of info.suggestions) {
      ...
      Assert.equal(
        ...
        `XXXXXXXXX ${content}`,
        `Expected suggestion to have displayurl: "XXXXXXXXX ${content}".`
      );
      index++;
    }
  ...
  let waitForConsole = new Promise(resolve => {
    SimpleTest.monitorConsole(resolve, [
      {
        message: new RegExp(
          `The keyword provided is already registered: "XXXXXXXXX"`
        ),
      },
    ]);
  });
